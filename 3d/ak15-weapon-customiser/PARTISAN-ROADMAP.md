...
3. **Rest of A**, then **B**, then **C**, then **D**, then **E**. *Progress: smoke test (`model-source/smoke-test.mjs`), credits panel, reduced motion and a first-run hint from E; presets, rifle camo (triplanar) and test fire (synthesised per-muzzle sound, flash, stat-driven recoil) from A; photo mode, loadout card and loadout codes from D; breathing, stat-driven sway, recoil in the hands and a Handling-timed reload from C.*
