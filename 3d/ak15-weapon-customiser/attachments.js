...
const plum=new T.MeshStandardMaterial({color:'#3e1f22',roughness:.6});
...
export const SLOTS=[
 {id:'muzzle',label:'Muzzle',options:[
  ...
  {id:'ak74',label:'AK-74 brake',detail:'Classic two-chamber AK-74 brake with its wide front port.',build:({materials})=>group(
  ...
  {id:'comp',label:'Compensator',detail:'Short three-port compensator; ports vent upward to hold the muzzle down.',build:({materials})=>group(
  ...
  {id:'can',label:'Suppressor',detail:'Full-length sound suppressor, 190 mm, on a quick-detach collar.',build:({materials})=>group(
 ...
 {id:'optic',label:'Optic',rail:{min:-.06,max:.04,step:.01},options:[
  ...
  {id:'holo',label:'Holographic',detail:'Box-hooded holographic sight with a wide window.',build:({materials})=>{
   const g=group(
  ...
  {id:'scope',label:'4× scope',detail:'4× fixed-power scope in two rings, 280 mm long.',build:({materials})=>group(
 ...
 {id:'foregrip',label:'Foregrip',rail:{min:-.08,max:0,step:.01},options:[
  ...
  {id:'angled',label:'Angled',detail:'Angled foregrip: a thumb ramp for a high, straight-arm hold.',build:({materials})=>group(
   [profile([[-.048,0],[.042,0],[.038,-.012],[-.028,-.046],[-.046,-.042]],.028,.003),materials.polymer])},
  {id:'stop',label:'Hand stop',detail:'Low hand stop at the front of the lower rail.',build:({materials})=>group(
 ...
 {id:'magazine',label:'Magazine',options:[
  ...
  {id:'45',label:'45-rnd RPK',detail:'45-round RPK-74 magazine: the 30-rounder lengthened by a third.',build:({original})=>reshape(original,v=>{if(v.y<0)v.y*=1.36;})},
  {id:'60',label:'60-rnd quad',detail:'60-round quad-stack: single-stack at the feed lips, twice as wide below the mag well.',build:({original})=>reshape(original,v=>{if(v.y<0)v.y*=1.18;v.z*=1+.85*smoothstep(.02,.06,-v.y);})},
  {id:'none',label:'None',detail:'Magazine removed.'}
 ]},
 {id:'grip',label:'Pistol grip',options:[
  {id:'rk9',label:'RK-9',original:true},
  {id:'classic',label:'Classic plum',detail:'Classic AK-74M-era plum polymer grip, slimmer and more steeply raked.',build:()=>group(
