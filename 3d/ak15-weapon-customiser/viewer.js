...
function updateStats(){
 const model=rifle.model,turned=model.rotation.y;model.rotation.y=0;model.updateMatrixWorld(true);
...
async function mount(id){
 ...
 document.querySelector('#rifle-title').textContent=config.title;
...
function restore(hash){
 ...
 queue=queue.then(async()=>{
  ...
  restoring=false;renderBuild();renderFinish();renderStats();writeHash();
...
try{
 ...
 function orientLights(){
  const r=controls.getAzimuthalAngle()-HERO.azimuth+T.MathUtils.degToRad(lightAngle);
 ...
 document.querySelector('#reset').onclick=()=>restore(rifle.id===DEFAULT_MODEL?'':'#rifle='+rifle.id).then(()=>view('hero'));
 ...
 function partAt(e){const r=renderer.domElement.getBoundingClientRect();pointer.set((e.clientX-r.left)/r.width*2-1,-(e.clientY-r.top)/r.height*2+1);ray.setFromCamera(pointer,camera);const hit=ray.intersectObject(rifle.model,true)[0];return hit&&rifle.parts.find(p=>p.objects.includes(hit.object));}
 ...
 renderer.setAnimationLoop(()=>{
  ...
  if(spinning){rifle.model.rotation.y+=dt*.5;socketMarkers.rotation.y=rifle.model.rotation.y;}
  stepTweens(dt);
