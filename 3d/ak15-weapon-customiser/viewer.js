...
// Lights follow the camera, so this relationship holds from every angle; LIGHT_OFFSET is in degrees.
...
try{
 ...
 // turned with the model so it still works mid-turntable.
 function frame(id){
  const slot=slots[id],target=slot.container.getWorldPosition(new T.Vector3());
  const {direction,distance,aim=[0,0,0]}=slot.spec.camera,up=new T.Vector3(0,1,0);
  target.add(new T.Vector3(...aim).applyAxisAngle(up,model.rotation.y));
  const dir=new T.Vector3(...direction).normalize().applyAxisAngle(up,model.rotation.y);
 ...
 new ResizeObserver(resize).observe(stage);resize();view('hero');
