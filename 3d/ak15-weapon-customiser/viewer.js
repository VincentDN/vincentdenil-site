...
function renderBuild(){
 ...
 buildPanel.replaceChildren(...Object.values(rifle.slots).map(slot=>{
  ...
  for(const o of slot.options){const b=document.createElement('button');b.textContent=o.label;b.setAttribute('aria-pressed',String(rifle.build[slot.spec.id]===o.id));b.onclick=()=>{applySlot(slot.spec.id,o.id,undefined,true);frame(slot.spec.id);};chips.append(b);}
...
// Live stats: length from the visible geometry (turntable angle ignored), mass from the options.
function updateStats(){
 ...
 const grams=rifle.config.baseGrams+Object.values(rifle.slots).reduce((sum,slot)=>sum+(slot.options.find(o=>o.id===rifle.build[slot.spec.id])?.grams||0),0);
