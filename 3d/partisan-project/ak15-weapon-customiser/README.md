...
URL: `/3d/partisan-project/ak15-weapon-customiser/` (moved from `/3d/ak15-weapon-customiser/`, which now redirects and keeps the loadout hash). Three screens: **Armoury** (two rifles, the AK-74M Zenitco and the AK-15K, with deep attachment customisation), **Operator** (a character creator) and **Field** (the operator holding the build). The plan it follows is `PARTISAN-ROADMAP.md`. Listed in `/3d/partisan-project/`, which `/projects/3d/` links to, with `seo_hidden=true` and robots `noindex`. Serve the repository root over HTTP; there is no build step. Uses Three.js 0.169.0 (OrbitControls, GLTFLoader, RGBELoader; newer than the other 3D pages because environment rotation needs r162+) and the shared `/assets/viewer-loader.css`. The viewer structure is adapted from `/3d/lousiana-flag-mount-test-xxl/`.

Plan: see `ROADMAP.md`. Current status: step 7 (round two) in progress; see `ROADMAP.md`.

Opening screen experiment: `/3d/partisan-project/ak15-workbench-intro/` is a TLOU II-style workbench intro that pushes in and fades to this viewer (see its README).
