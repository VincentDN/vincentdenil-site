...
function preset(type){controls.autoRotate=false;document.querySelector('#rotate').setAttribute('aria-pressed','false');const v=type==='front'?[0,0,1]:type==='edge'?[0,-.95,.22]:[.16,.42,1];camera.position.set(...v).normalize().multiplyScalar(baseDistance);controls.target.set(0,0,.0015);controls.update();}
...
for(const id of ['front','edge','perspective'])document.getElementById(id).onclick=()=>preset(id);
...
['3.35 mm*',[.05,-.032,.00335],'Assumed thickness','3.35 mm maximum: 2 mm base plus up to 1.35 mm emblem relief. Depth was not supplied in the reference.']];
